...
public class GetTaskByIdQueryValidator : AbstractValidator<GetTaskByIdQuery>
{
    ...
    public GetTaskByIdQueryValidator(IApplicationDbContext context, IUser user)
    {
        _context = context;
        _user = user;

        RuleFor(v => v.Id).MustAsync(TaskExists).WithMessage("Task isnt found");
        if (_user.Role != Roles.Administrator)
        {
            RuleFor(v => v.Id).MustAsync(AssignedUserTask).WithMessage("User isnt Assigned to the task");
        }
    }
